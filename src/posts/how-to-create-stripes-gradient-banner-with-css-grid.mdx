...
Want to jump straight in to the code? Check out the solution on <a href="https://codepen.io/lukejohnbrown/pen/WNQbMzO" target="_blank">Codepen</a>.
...
Although hopefully it goes without saying, the design and colors used within this post are all borrowed and inspried by <a href="https://stripe.com">stripe.com</a>.
...
Although Stripe uses absolute positioning to here (which is likely for better browser support), this is actually pretty straight forward to achieve with CSS Grid.

<Spacer space="4" />

## The Solution with CSS Grid

When building with CSS grid, it helps to visualise the layout as a grid (which sounds obvious when you say it out loud). As you can see, the Stripe banner is a grid made up of `6` columns and `4` rows:
...
We are rotating our banner on the `Y` axis by `-12` degrees and specifying that the transform applies to the top left of our banner (remove the `transform-origin` to see the default behaviour!).
